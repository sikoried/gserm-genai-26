...
_SYSTEM = (
    ...
    "call another tool.\n"
    "- **Do NOT repeat a tool call that already appears under 'Tools used so far' "
    "with the same or a very similar query — it wastes a step. Either try a "
    "DIFFERENT tool or a clearly different query, or finish.**\n"
    "- Escalation order when the answer is still missing: `search`/`wiki_lookup` "
    "(local) → `google_search` (the web) → `youtube` (video transcripts).\n"
    ...
    "- Use `youtube` when the question refers to a video, AND as a FALLBACK: if "
    "`search` and `google_search` returned no clear answer (empty or unhelpful), try "
    "`youtube` before giving up — video transcripts often contain answers the other "
    "tools miss.\n"
    ...
    "soon as the question can be answered."
)
